...
export class DeliberationEventEmitter extends EventEmitter {
  private deliberationListeners: Map<string, IDeliberationEventListener[]> = new Map();
  ...
  registerListener(contractId: string, listener: IDeliberationEventListener): void {
    if (!this.deliberationListeners.has(contractId)) {
      this.deliberationListeners.set(contractId, []);
    }
    this.deliberationListeners.get(contractId)!.push(listener);
  ...
  unregisterListener(contractId: string, listener: IDeliberationEventListener): void {
    const contractListeners = this.deliberationListeners.get(contractId);
    if (contractListeners) {
      ...
      if (contractListeners.length === 0) {
        this.deliberationListeners.delete(contractId);
      }
    }
  ...
  getListenerCount(contractId: string): number {
    return (this.deliberationListeners.get(contractId) || []).length + this.listenerCount(`message:${contractId}`);
  }
